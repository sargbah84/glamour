...
class UserEventListener
{
    ...
    public function onUpdated($event)
    {
        activity('user')
            ...
            ->log(':causer.firstname updated user :subject.firstname with roles: :properties.roles and permissions: :properties.permissions');
    }
    ...
    public function onDeleted($event)
    {
        activity('user')
            ->performedOn($event->user)
            ->log(':causer.firstname deleted user :subject.firstname');
    }
    ...
    public function onRestored($event)
    {
        activity('user')
            ->performedOn($event->user)
            ->log(':causer.firstname restored user :subject.firstname');
    }
    ...
    public function onDestroyed($event)
    {
        activity('user')
            ->performedOn($event->user)
            ->log(':causer.firstname permanently deleted user :subject.firstname');
    }
    ...
    public function onStatusChanged($event)
    {
        activity('user')
            ->performedOn($event->user)
            ->log(':causer.firstname '.($event->status === 0 ? 'deactivated' : 'reactivated').' user :subject.firstname');
    }
