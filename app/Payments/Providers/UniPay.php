...
class UniPay extends AbstractProvider implements Provider
{
    ...
    public function redirect($transaction)
    {
        ...
        $uniPayOrder = [
            ...
            'SuccessRedirectUrl' => base64_encode(route('frontend.user.account.order.callback', 'unipay') . '?order_id=' . $transaction->id),
            'CancelRedirectUrl' => base64_encode(route('frontend.user.account.order.callback', 'unipay') . '?order_id=' . $transaction->id),
